...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("kafka_log.log"), logging.StreamHandler()],
)
c = Config()
...
def get_notification_log_between_timestamp(
    session: sessionmaker, dt_from: datetime.datetime, dt_to: datetime.datetime
):
...
def get_topic_subscriptions_between_timestamp(
    session: sessionmaker, dt_from: datetime.datetime, dt_to: datetime.datetime
):
...
def get_sensor_data(session: sessionmaker, topic_id: int):
    ...
    try:
        mqtt_data = (
            session.query(Mqtt)
            .order_by(Mqtt.id.desc())
            .filter(Mqtt.topic_id == topic_id)
            .first()
        )
    except Exception as e:
        logging.exception(e)
...
def get_sensor_data_between_timestamp(
    ...
    cond = and_(
        Mqtt.timestamp.between(str(dt_from), str(dt_to)), Mqtt.topic_id == topic_id
    )
    return (
        session.query(Mqtt)
        .with_entities(Mqtt.id, Mqtt.topic, Mqtt.value, Mqtt.timestamp, Mqtt.topic_id)
        .filter(cond)
        .all()
    )
    ...
    while True:
        ...
        try:
            ...
            if current_time > every_minute:
                ...
                date_to = date_from + datetime.timedelta(minutes=1, microseconds=-1)
            ...
            if current_time > every_minute2:
                ...
                print(
                    f"Getting Topic Subscription data from {str(date_from)} to {str(date_to)}"
                )

                # Get Topic Subscriptions
                topic_subs = get_topic_subscriptions_between_timestamp(
                ...
                notification_logs = get_notification_log_between_timestamp(
                    session, date_from, date_to
                )
